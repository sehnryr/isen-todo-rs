...
use crate::model::db::{List, Session, Task, User};
...
// lists
impl Repository {
    pub async fn create_list(&mut self, title: String, created_by: Uuid) -> Result<()> {
        let created_at = Utc::now();

        sqlx::query!(
            "INSERT INTO lists (title, created_at, created_by)
            VALUES (?, ?, ?)",
            title,
            created_at,
            created_by
    ...
    pub async fn get_lists_by_user(&mut self, user_id: Uuid) -> Result<Vec<List>> {
    ...
    pub async fn get_list_tasks(&mut self, list_id: Uuid) -> Result<Vec<Task>> {
        let rows = sqlx::query_as!(
            ...
            r#"SELECT
                ...
                completed_by as "completed_by: _"
            FROM tasks
            WHERE list_id = ?"#,
            list_id
        )
